...
import type { GazePoint, InputMethod, FaceEvent } from '../types'
...
export function TutorialPage({ gazeOffsetX = 0, gazeOffsetY = 0, onNext }: Props) {
  const { t } = useI18n()
  ...
  // Setup face detector + webcam once
  useEffect(() => {
    ...
    const unsub = detector.onFace(face => {
      setFaceEvent(face)
  ...
  // Setup gaze layer once
  useEffect(() => {
    const gLayer = gazeLayerRef.current
    gLayer.setOffset(gazeOffsetX, gazeOffsetY)
  ...
  useEffect(() => {
    ...
    const unsub = ctrl.onInput(() => {
      // Auto-advance on any input during practice
  ...
  const advancePractice = () => {
    if (practiceIdx < PRACTICE_CHARS.length - 1) {
      setPracticeIdx(i => i + 1)
      return
    }
    setPracticeIdx(0)
  ...
  // --- Practice screens (with keyboard + gaze) ---
  const ctrl = controllerRef.current
  const targetChar = PRACTICE_CHARS[practiceIdx]

  // Reserve ~35% of screen height for title, instruction, target char, progress, skip button
